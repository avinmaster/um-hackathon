...
const cellRing: Record<StepStatus, string> = {
  pending: "",
  running: "shadow-[0_0_12px_-2px_var(--color-primary-glow)]",
  awaiting_user: "shadow-[0_0_10px_-2px_var(--color-warn)]",
  passed: "",
  failed: "shadow-[0_0_10px_-2px_var(--color-fail)]",
};
...
export function ProgressStrip({
  ...
  return (
    <div className="flex flex-col gap-2 border-b border-[var(--color-border)] bg-[var(--color-bg-elev)] px-4 py-3">
      <div className="flex items-center justify-between">
        ...
        <span className="font-mono text-[11px] text-[var(--color-primary-glow)]">
          {pct}%
        </span>
      </div>
            ...
            >
              {isCurrent && (
                <motion.span
                  ...
                  className="pointer-events-none absolute -bottom-3 left-1/2 -translate-x-1/2 h-1 w-1 rounded-full bg-[var(--color-primary-glow)]"
                  transition={{ type: "spring", stiffness: 500, damping: 30 }}
                />
              )}
              <span className="pointer-events-none absolute -top-7 left-1/2 -translate-x-1/2 whitespace-nowrap rounded border border-[var(--color-border)] bg-[var(--color-bg-translucent)] px-1.5 py-0.5 text-[10px] text-[var(--color-ink-muted)] opacity-0 transition-opacity group-hover:opacity-100 backdrop-blur z-10">
                {i + 1}. {n.title}
              </span>
            </button>
          );
